 ...
 * Copyright (c) 2009-2010, 2012, 2020 The Linux Foundation. All rights reserved.
 *
...
#define AMRWBPLUSTEST_IBUFSZ (32*1024)
#define AMRWBPLUSTEST_NUM_IBUF 2
#define AMRWBPLUSTEST_IPMEM_SZ (AMRWBPLUSTEST_IBUFSZ * AMRWBPLUSTEST_NUM_IBUF)

#define AMRWBPLUSTEST_OBUFSZ (32*1024)
#define AMRWBPLUSTEST_NUM_OBUF 2
#define AMRWBPLUSTEST_OPMEM_SZ (AMRWBPLUSTEST_OBUFSZ * AMRWBPLUSTEST_NUM_OBUF)
...
struct msm_audio_aio_buf aio_ip_buf[AMRWBPLUSTEST_NUM_IBUF];
struct msm_audio_aio_buf aio_op_buf[AMRWBPLUSTEST_NUM_OBUF];
...
static void *amrwbplus_read_thread(void *arg)
{
	struct audio_pvt_data *audio_data = (struct audio_pvt_data *) arg;
	int afd = audio_data->afd;
	int total_len;
...
static void *amrwbplus_write_thread(void *arg)
{
	struct msm_audio_aio_buf aio_buf;
	struct audio_pvt_data *audio_data = (struct audio_pvt_data *) arg;
...
static void *amrwbplus_dec_event(void *arg)
{
        struct audio_pvt_data *audio_data = (struct audio_pvt_data *) arg;
	...
	if (audio_data->mode) // Non Tunnel mode
		pthread_create(&evt_read_thread, NULL, amrwbplus_read_thread, (void *) audio_data);
	pthread_create(&evt_write_thread, NULL, amrwbplus_write_thread, (void *) audio_data);
...
static int initiate_play(struct audtest_config *clnt_config)
{
	...
	struct mmap_info *in_ion_buf[AMRWBPLUSTEST_NUM_IBUF];
    struct mmap_info *out_ion_buf[AMRWBPLUSTEST_NUM_OBUF];

        audio_data->freq = 48000;
        audio_data->channels = clnt_config->channel_mode;
        audio_data->bitspersample = 16;

	if(((in_size + sizeof(struct meta_in_q6)) > AMRWBPLUSTEST_IBUFSZ) ||
		(out_size > AMRWBPLUSTEST_OBUFSZ)) {
	...
	afd = open("/dev/msm_amrwbplus", open_flags);

	if (afd < 0) {
		perror("Cannot open amrwbplus device");
		return -1;
	}
	...
	if ((ioctl(afd, AUDIO_START, 0))< 0 ) {
		printf("amrwbplustest: unable to start driver\n");
		goto err_state2;
	}
	if (audio_data->mode) {
		/* non - tunnel portion */
		...
		for (n = 0; n < AMRWBPLUSTEST_NUM_OBUF; n++) {

			out_ion_buf[n] = alloc_ion_buffer(ionfd, AMRWBPLUSTEST_OBUFSZ);
			if (!out_ion_buf[n]) {
		...
		for (n = 0; n < (AMRWBPLUSTEST_NUM_OBUF-1); n++) {
		...
		out_free_indx = AMRWBPLUSTEST_NUM_OBUF-1;
	}
	//Register Write  buffer
	for (n = 0; n < AMRWBPLUSTEST_NUM_IBUF; n++) {
		in_ion_buf[n] = alloc_ion_buffer(ionfd, AMRWBPLUSTEST_OBUFSZ);
		if (!out_ion_buf[n]) {
			printf("\n alloc_ion_buffer: out_ion_buf[n] allocation failed\n");
		    goto err_state2;
		}
	...
	//Indicate available free buffer as (n-1)
	in_free_indx = AMRWBPLUSTEST_NUM_IBUF-1;
	pthread_create(&evt_thread, NULL, amrwbplus_dec_event, (void *) audio_data);
...
static int play_file(struct audtest_config *config, int fd, unsigned count)
{
	...
	ret_val = initiate_play(config);
	free(content_buf);
	return ret_val;
}

int amrwbplus_play(struct audtest_config *config)
{
	struct stat stat_buf;
	int fd;

	fprintf(stderr,"Inside amrwbplus_play \n");
	...
	if (fd < 0) {
		fprintf(stderr, "playamrwbplus: cannot open '%s'\n",
				config->file_name);
		return -1;
	}
...
void *playamrwbplus_thread(void *arg)
{
	struct audiotest_thread_context *context =
		(struct audiotest_thread_context *) arg;
	int ret_val;

	ret_val = amrwbplus_play(&context->config);
...
int amrwbplusplay_read_params(void* filedata)
{
	struct audiotest_thread_context *context;
...
int main(int argc, char* argv[])
{
	...
	amrwbplusplay_read_params((void *)&stream_param[i]);

	return 0;
}


const char *amrwbplusplay_help_txt =
"Play amrwbplus file: type \n\
echo \"playamrwbplus path_of_file -id=xxx -mode=x -cmode=x\
...
void amrwbplusplay_help_menu(void)
{
	printf("%s\n", amrwbplusplay_help_txt);
}
